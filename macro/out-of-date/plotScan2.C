  ...
  const char* finPath = Form("%s/%s", dirName, finName); 
	...
	std::vector<std::vector<std::vector<TH1D*>>> vvvhQ; //( chanN, std::vector<std::vector<TH1D*>>
			                                            //( param0_nb, std::vector<TH1D*> 
																								  //( param1_nb, nullptr ) ) );
	std::vector<std::tuple<int, int, int, TH1D*>> vvvhQ; //( chanN, std::vector<std::vector<TH1D*>>
	int i=0;
	...
	for( int chIdx=0; chIdx<4; chIdx++ )
	{
		...
		for( int pIdx0=0; pIdx0<param0_nb; pIdx0++ )
		{
		  ...
		  {
			  TString hname = Form( "hQ_ch%d_%s%d_%s%d", 
						                      activeChannel[chIdx], 
																  pname0.Data(), (int) params0[pIdx0],
																  pname1.Data(), (int) params1[pIdx1] );
				//cout << Form("%02d: hname= %s | pname0= %s, pname1= %s", i++, hname.Data(), pname0.Data(), pname1.Data() ) << endl;
				
			  TH1D* htemp = new TH1D( hname.Data(), "", 80, 0, 320 );
				vhQ.push_back( htemp );
			}
			vvhQ.push_back( vhQ );
		}
	...
	//	{
	//		for( auto h: vh )
	//			cout << Form("%02d: hname= %s",i++,h->GetName()) << endl;
	//	}
	//} 
	...
	for( int i=0; i<entries; i++ )
	{
		...
		vvvhQ.at(chIdx).at(pIdx0).at(pIdx1)->Fill( efine );
		  ...
		  for( int pidx1=0; pidx1<param1_nb; pidx1++ )
			{
				...
				vvvhQ.at(ch).at(pidx0).at(pidx1)->SetTitle( Form("%s=%.0f, %s=%.0f;efine (QDC in ADC);Counts", param0_name.Data(), params0[pidx0], param1_name.Data(), params1[pidx1]) );
				vvvhQ.at(ch).at(pidx0).at(pidx1)->Write();
			}
		}
	}
	...
	for( int ch=0; ch<chanN; ch++ )
	{
	  ...
	  for( int pidx1=0; pidx1<param1_nb; pidx1++ )
	  {
			...
			gtempE->SetName( Form("gEntr_ch%03d_%s_%.0f", chanID, pname1.Data(), params1[pidx1]) );
			//gtempE->SetTitle( Form("ch%03d_%s_%.0f", chanID, param1_name.Data(), params1[pidx1]) );
			
			auto gtempM = new TGraphErrors();
			gtempM->SetName( Form("gMean_ch%03d_%s_%.0f", chanID, pname1.Data(), params1[pidx1]) );
	    ...
	    for( int pidx0=0; pidx0<param0_nb; pidx0++ )
	  	{
	  		int canvIdx = pidx1*param0_nb + pidx0 + 1;
				...
				c1->cd( canvIdx );
				vvvhQ.at(ch).at(pidx0).at(pidx1)->Draw();
